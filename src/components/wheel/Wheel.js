...
export default class Wheel extends Component {
  ...
  onPanResponderGrant(e, gestureState) {
      ...
      this.prevTouches = e.nativeEvent.touches;//记录触碰的位置
      this.speed = 0;
  }
  ...
  handleSwipeScroll() {
      ...
      this.setAnimated(newIndex);
  }

  handleStopScroll() {
      this.setAnimated(this.index);
  }

  setAnimated = (index)=>{
      let toValue = index * this.holeHeight;
  ...
  render() {
      let {items, index, ...others} = this.props;
      this.lastRenderIndex = this.index;
      return (
          <View
              {...others}
